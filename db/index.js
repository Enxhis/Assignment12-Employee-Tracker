...
class DataBase {
    ...
    findEmployeesByDepartment(departmentId) {
        return this.connection.query(
            "SELECT employee.id, employee.firstname, employee.lastname, role.title FROM employee LEFT JOIN role ON employee.role_id = role.id LEFT JOIN department ON role.department_id = department.id WHERE department.id = ? ;", departmentId
        );
    }
    ...
    findDepartments() {
        return this.connection.query(
            ...
            //"SELECT department.id, department.name, SUM(role.salary) AS utilized_budget FROM role LEFT JOIN department ON role.department_id = department.id GROUP BY department.id, department.name;"
        );
    }
    // create new employee
